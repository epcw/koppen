...
station_list = open("stations/station_ids.txt",
                    'r').read().splitlines()  # if you've had to start / stop the script, make sure to save a station_ids-COMPLETE.txt file
...
for s in station_list:  # iterate over station list
    for d in dates:  # iterate over date range (have to chunk this because there is the 1000 record limit per pull)
        url = "https://www.ncdc.noaa.gov/cdo-web/api/v2/data?datasetid=GSOM&stationid=" + s + "&startdate=" + str(
            d) + "-01-01&enddate=" + str(
            d) + "-12-31&limit=1000&units=metric&datatypeid=PRCP,TAVG"  # NOTE 10 years at a time max
        ...
        if resp.status_code == 429:
            ...
            try:
                ...
                print("skipping " + s + " " + str(
                    d))  # handle stations that are not available for the entire length of the pull
            ...
            print("service unavailable (503) on " + s + " " + str(d) + ". Retrying.")
            resp = requests.get(url, headers=headers)
            r = json.loads(
                resp.text)  # don't stick this inside the try because when it fails, you know you've run out of requests for the day (10k/day)
            try:
